...
public class Door2 : MonoBehaviour 
{
	...
	public AudioClip doubleDoorOpened;
	public AudioClip doubleDoorClosed;
	...
	float timer = 0.0f;
	public bool secondKeyFound = false;
			...
			if (Physics.Raycast(rayOrigin, fpsCam.transform.forward, out hit, reachRange))
			{
				...
				if (moveableObject != null)		//hit object must have MoveableDraw script attached
				{
					...
					if (Input.GetKeyUp(KeyCode.E) || Input.GetButtonDown("Fire1"))
					{
						...
						msg = getGuiMsg(!isOpen);
						Door2_Hint_2.enabled = true;
						GetComponent<AudioSource>().PlayOneShot(doubleDoorOpened);
						GameObject.Find("FPSController").SendMessage("SecondKeyUsed");
						
						}else{
							Door2_Hint_1.enabled = true;
							
							Debug.Log("Brak klucza");
							GetComponent<AudioSource>().PlayOneShot(doubleDoorClosed);
						}
					}
